...
# 第一部分：全局配置区 (iPhone 14 Pro 专用版)

# ================================================================================

# 数据来源整合：

# 1. 电池特性：NASA PCoE B0005 拟合数据 (OCV) + 典型高压 LCO 化学特性

# 2. 硬件规格：iFixit 拆解报告 (iPhone 14 Pro)

# 3. 功耗模型：Geekerwan A16 芯片能效评测 + DisplayMate 屏幕测试
...
        "Ea": 52000.0,         # [修正] 内阻活化能。锂钴氧化物(LCO)体系通常比磷酸铁锂更高，取 52kJ/mol

        "cp": 1040.0,          # [修正] 电池比热容。软包锂电池典型值约为 1040 J/(kg·K)

        "h": 8.0,              # 对流换热系数。手机被动散热（无风扇）典型值

        "T_env_default": 25.0, # 默认环境温度 [°C]

        "T_ref": 298.15,       # 参考温度 25°C [K]

        "E_aging": 45000.0,    # 老化活化能 [J/mol]，用于控制高温下的 SOH 衰减速率
...
        "C_nom_mAh": 3200.0,   # [实测] 额定容量。注意：14 Pro 为 3200mAh (12.38Wh)

        "mass": 0.055,         # [估算] 电池质量。整机 206g，电池约占 27%，约 55g

        "area": 0.0105,        # [计算] 散热面积。机身 147.5x71.5mm，取单面投影面积

        "V_min": 3.0,          # [设定] 截止电压。依据你的拟合曲线，3.0V 是安全截止点

        "V_max": 4.45,         # [特性] 满充电压。现代旗舰机电池多为高压体系
...
        "R_ref": 0.035,        # [优化] 参考内阻。高性能旗舰机电池内阻通常较低 (35mΩ)

        "beta_pol": 0.015,     # 极化内阻系数。控制低电量时内阻抬升的幅度

        "k_ref": 1.0e-8,       # 老化速率。对应约 500-800 循环后的寿命衰减

        "T_opt": 298.15,       # 最佳工作温度

        "k_soc": 1.6,          # SOC 极化指数。控制电压崩塌的陡峭程度
...
    # 功耗模型参数 (A16 Bionic & Super Retina XDR)
...
        "P_base": 0.45,        # [W] 待机底噪 (基带+协处理器)

  

        # 屏幕功耗 (6.1" OLED LTPO)

        "P_panel_base": 0.4,   # [W] 最低亮度/AOD 功耗

        "k_scr": 3.2,          # [W] 亮度系数。1000nits 下整机屏幕功耗约 3.6W

        "beta_screen": 1.6,    # [W] OLED 非线性指数，通常在 1.6 左右

  

        # CPU 功耗 (A16 Bionic, TSMC 4nm)

        "P_leak_ref": 0.3,     # [W] 静态漏电

        "lambda_leak": 0.015,  # 漏电温敏系数。先进制程对温度更敏感

        "k_cpu": 3.5,          # [W] 动态功耗系数。满载 (SOC+DRAM) 峰值约 10-11W

        "T_crit": 318.15,      # [K] 45°C。iOS 严重热节流阈值
...
        "bg_period": 60.0,     # 唤醒周期 [s]

        "bg_on": 5.0,          # 唤醒持续时间 [s]

        "P_gps": 0.5,          # 双频 GPS 功耗

        "gps_period": 30.0,    # 定位周期 [s]

        "gps_on": 10.0,        # 定位持续时间 [s]
...
            "5G": 1.4,         # 5G NSA 模式功耗显著增加

            "WiFi": 0.8,       # WiFi 6 相对省电
...
    # OCV 多项式系数 (来自 NASA B0005 数据拟合)

    # ------------------------------------------------------------------------

    # 对应: V = c[0]*x^5 + c[1]*x^4 + ... + c[5]

    # 你刚刚拟合得到的数据，已按高阶到低阶排序

    # ------------------------------------------------------------------------

    "ocv_coeffs": [-5.2318, 9.4303, -1.2573, -5.6007, 3.778, 2.777],
...
        "mu": 0.0,             # 信号强度均值 (dBm 归一化)

        "theta": 0.2,          # 回归速度

        "sigma": 0.5,          # 波动标准差，模拟移动时的信号不稳
...
  "simulation_date": "2026-01-30 20:16:16",
