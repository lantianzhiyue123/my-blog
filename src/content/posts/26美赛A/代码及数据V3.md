...
# 第一部分：全局配置区 (iPhone 14 Pro - 2026 美赛专用版)

# ================================================================================

# 数据来源：基于 3uTools 实测、Apple 官方规格及拆解报告。

# 核心特征：

# 1. 高压 LCO 体系 (4.47V Max)。

# 2. A16 Bionic 能效模型 (3.5W 归一化峰值)。
...
        "Ea": 35000.0,         # [确认] 活化能 35kJ/mol。

                               # 依据：数值稳健性优化，防止 -10°C 下内阻爆炸导致模型发散。

        "cp": 1040.0,          # [未更新] 电池比热容 [J/(kg·K)] (保留通用锂电池值)

        "h": 8.0,              # [确认] 对流换热系数 (7.0-8.0 范围内取高值以模拟微弱空气流动)

        "T_env_default": 25.0, # 默认环境温度 [°C]

        "T_ref": 298.15,       # 参考温度 25°C [K]

        "E_aging": 45000.0,    # [未更新] 老化活化能 (保留默认)
...
        "C_nom_mAh": 3200.0,   # [确认] 额定容量 (12.38Wh / 3.87V approx)

        "mass": 0.044,         # [修正] 电池质量 44g (原 55g)。依据：拆解报告单体净重。

        "area": 0.0105,        # [确认] 有效散热面积 (背部投影)。

        "V_min": 3.0,          # 截止电压 (OCV 0% 为 3.4V，3.0V 为安全负载截止点)

        "V_max": 4.47,         # [修正] 满充电压。依据：3uTools 读取高压 LCO 体系极限。
...
        "R_ref": 0.035,        # [确认] 参考内阻。依据：旗舰机电芯低阻特性 (0.035-0.045范围取下限)。

        "beta_pol": 0.015,     # [未更新] 极化内阻系数 (保留默认)

        "k_ref": 1.0e-8,       # [未更新] 老化速率 (保留默认)
...
        "P_base": 0.05,        # [确认] 系统底噪 50mW。依据：iOS 严格后台控制。

        "P_panel_base": 0.1,   # [未更新] 屏幕基础电路功耗 (保留默认)

        "k_scr": 3.2,          # [确认] 屏幕亮度系数。依据：满亮总功耗 ~3.3W。

        "beta_screen": 1.6,    # [未更新] 亮度非线性指数 (保留 OLED 特性)

  

        # 处理器 (A16)

        "P_leak_ref": 0.05,    # [未更新] CPU 静态漏电 (保留默认)

        "lambda_leak": 0.015,  # [未更新] 漏电温度系数

        "k_cpu": 3.5,          # [确认] 处理器功耗系数。依据：Geekbench 归一化负载。

        "T_crit": 318.15,      # [确认] 热节流温度 45°C。
...
        # 脉冲任务配置 (保留默认，除非有具体 App 行为数据)

        "P_wake": 0.3, "N_app": 6, "bg_period": 60.0, "bg_on": 5.0,

        "P_gps": 0.5, "gps_period": 30.0, "gps_on": 10.0,
...
            "5G": 1.4,         # [确认] 5G 功耗倍率 1.4。

            "WiFi": 0.8,       # [确认] WiFi 功耗倍率 0.8。
...
    # 公式: V = 0.6*s^3 - 0.4*s^2 + 0.87*s + 3.4

    # 验算 SOC=1.0: 0.6 - 0.4 + 0.87 + 3.4 = 4.47V (完美匹配 iPhone 14 Pro)

    # 验算 SOC=0.0: 3.40V (符合关机电压)

    # ------------------------------------------------------------------------

    "ocv_coeffs": [0.6, -0.4, 0.87, 3.4],
...
        # 说明：真实电池在极端条件下，保护电路会介入防止损坏。

        #       这里设置 3Ω 上限，防止低温+低SOC时内阻计算值溢出。
...
[DEBUG] 电压崩溃! SOC=0.1028, T=316.70K
        Voc=3.47V, R=0.5841Ω, P_total=5.51W
        Voc²=12.07, 4RP=12.88
        判别式=-0.8179 (需 > 0 才能正常工作)
[DEBUG] 电压崩溃! SOC=0.1030, T=316.69K
        Voc=3.47V, R=0.5828Ω, P_total=5.52W
        Voc²=12.07, 4RP=12.86
        判别式=-0.7937 (需 > 0 才能正常工作)
  -> 详细图已保存: e:\python\pythonProject2\anything\26美赛\simulation_results\Case_B_detailed.png

正在仿真: Case_C (极寒场景 (Case C))...
[DEBUG] 电压崩溃! SOC=0.1013, T=266.55K
        Voc=3.47V, R=0.7694Ω, P_total=4.01W
        Voc²=12.06, 4RP=12.35
        判别式=-0.2874 (需 > 0 才能正常工作)
[DEBUG] 电压崩溃! SOC=0.1013, T=266.55K
        Voc=3.47V, R=0.7692Ω, P_total=4.01W
        Voc²=12.06, 4RP=12.34
        判别式=-0.2842 (需 > 0 才能正常工作)
...
--------------------------------------------------------------------------------
办公场景 (Case A)             8.00       25.1         25.0         0.319
游戏场景 (Case B)             1.93       43.5         35.0         1.618
极寒场景 (Case C)             5.43       -6.6         -10.0        0.629
================================================================================
